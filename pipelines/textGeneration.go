...
func WithBeamSearch(enable bool) pipelineBackends.PipelineOption[*TextGenerationPipeline] {
	return func(pipeline *TextGenerationPipeline) error {
		if pipeline.Runtime == "ORT" {
			// o.ORTOptions.BeamSearchEnabled = &enable
			pipeline.BasePipeline.Model.BeamSearchEnabled = true
			return nil
		} else {
...
func WithBeamSearchNumBeams(numBeams int) pipelineBackends.PipelineOption[*TextGenerationPipeline] {
	return func(pipeline *TextGenerationPipeline) error {
		if pipeline.Runtime == "ORT" {
			if numBeams < 2 {
				return fmt.Errorf("numBeams must be at least 2, got %d", numBeams)
			}
			pipeline.BasePipeline.Model.BeamSearchNumBeams = numBeams
			return nil
		} else {
